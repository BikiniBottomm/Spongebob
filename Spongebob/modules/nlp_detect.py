...
@pbot.on_message(filters.command("spamprotection"), group=8)
        ...
        if len(args) > 1:
            if args[1].lower() in ["on", "yes"]:
                ...
                await message.reply_text(
                    "I've enabled Spam Protection moderation in this group. This will help protect you "
                    "from spammers, unsavoury characters, and the biggest trolls."
                )
            elif args[1].lower() in ["off", "no"]:
                sql.disable_nlp(message.chat.id)
                await message.reply_text(
                    "I've disabled Spam Protection moderation in this group. NLP wont affect your users "
        ...
        try:
            ...
            if res_json['success']:
                ...
                if spam_check == True:
                    ...
                    await pbot.restrict_chat_member(chat.id, user.id, ChatPermissions(can_send_messages=False))
                    try:
                        await message.reply_text(
