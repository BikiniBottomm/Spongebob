...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is part of the Nations\nI can't act against our own."
        )
        return

    if int(user_id) in DRAGONS:
        message.reply_text(
            "I spy, with my little eye... a nations! Why are you guys turning on each other?"
        )
        return

    if int(user_id) in DEMONS:
        message.reply_text(
            "OOOH someone's trying to gban a Patrick nations! *grabs popcorn*"
        )
        return

    if int(user_id) in TIGERS:
        message.reply_text("That's a Pearl! They cannot be banned!")
        return

    if int(user_id) in WOLVES:
        message.reply_text("That's a squidward! They cannot be banned!")
        return
...
def gbanstat(update: Update, context: CallbackContext):
    ...
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text(
                "Spamwatch is now disabled ❌ " "Spamwatch is now disabled ❌"
            )
    else:
        update.effective_message.reply_text(
...
__help__ =f"""
*Admins only:*
• `/spamwatch <on/off/yes/no>`*:* Will toggle our antispam tech or return your current settings.
• `/spamprotection <on/off/yes/no>`*:* Will toggle our Spam Protection tech or return your current settings.
• `/nsfwdetect <on/off/yes/no>`*:* Will toggle our NSFW Classification tech or return your current settings
Anti-Spam, used by bot devs to ban spammers across all groups. This helps protect
you and your groups by removing spam flooders as quickly as possible.
*Note:* Users can appeal gbans or report spammers at @VohaChat
...
*What is NSFW Classification?*
NSFW Classification is the new AI anti nsfw service which makes sure that your chat is free of nsfw content
Uses @Intellivoid's Coffeehouse Artificial Engine. Every ban is checked by real trusty people before being finalized.
...
GBAN_STATUS = CommandHandler("spamwatch", gbanstat, filters=Filters.group)
